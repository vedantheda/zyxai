...
export default function WorkflowsPage() {
  ...
  const fetchWorkflowData = async () => {
    try {
      ...
      // Calculate stats
      const workflowStats = {
        total: workflowsData?.length || 0,
        active: workflowsData?.filter(w => w.status === 'in_progress' || w.status === 'pending').length || 0,
        completed: workflowsData?.filter(w => w.status === 'completed').length || 0,
        failed: workflowsData?.filter(w => w.status === 'failed').length || 0,
        pendingAlerts: alertsData?.filter(a => a.status === 'pending').length || 0,
        clientsWithActiveWorkflows: new Set(
          workflowsData?.filter(w => w.status === 'in_progress' || w.status === 'pending').map(w => w.client_id)
        ).size || 0
      }
