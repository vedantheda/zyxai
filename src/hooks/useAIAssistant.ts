...
export function useAIAssistant(options: UseAIAssistantOptions = {}) {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      if (error) {
        console.warn('Database error loading conversations:', error)
...
What would you like help with today?`,
      ...
      try {
        ...
        if (error) {
          console.warn('Database save failed, continuing with local state:', error)
    ...
    try {
      ...
      if (!conversationToUse) {
        try {
          ...
          console.warn('Failed to create conversation, proceeding without one:', createError)
      ...
      try {
        ...
        console.warn('Background sync failed:', loadError)
      }
...
• Effective Rate: ${result.effectiveRate.toFixed(2)}%`
  ...
  const deleteConversation = useCallback(async (conversationId: string): Promise<void> => {
    ...
    try {
      ...
      console.warn('Error deleting conversation from database:', err)
  ...
  const updateConversationTitle = useCallback(async (conversationId: string, title: string): Promise<void> => {
    ...
    try {
      ...
      console.warn('Error updating conversation title in database:', err)
